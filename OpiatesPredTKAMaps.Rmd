...
#MED/Month Map Data - Might want to do median instead of mean
MEDMapDF <- summarize(datagrp, Mean_MED_per_Month = mean(MEDperMonth), Count = n())
rmSmallSamp2 <- function() {
        for (i in 1:nrow(MEDMapDF)) {
                if (MEDMapDF$Count[i] < 10) {
                        MEDMapDF$Mean_MED_per_Month[i] <- NA
                } else {next}
        }
        MEDMapDF
}
...
Create Revision Map
```{r}
j <- ggplot() + geom_polygon(data = counties.df2, aes(x = long, y = lat, group = group, fill = Revision_Rate), color = "black", size = 0.05)+ coord_map() + scale_fill_distiller(palette = "Greens", labels = percent, breaks = pretty_breaks(n = 10)) + guides(fill = guide_legend(reverse = TRUE)) + theme_nothing(legend = TRUE) + labs(title = "TKA Revision Rates", fill = "") + theme(plot.title = element_text(size=20, family = "Times"),legend.key.height = unit(.35, "cm"),legend.position = c(.9, .3))
...
```{r}
p <- ggplot() + geom_polygon(data = counties.df2, aes(x = long, y = lat, group = group, fill = Mean_MED_per_Month), color = "black", size = 0.05)+ coord_map() + scale_fill_distiller(palette = "Greens", breaks = pretty_breaks(n = 10)) + guides(fill = guide_legend(reverse = TRUE)) + theme_nothing(legend = TRUE) + labs(title = "Mean Morphine Equivalents per Month per Patient", fill = "") + theme(plot.title = element_text(size=20, family = "Times"),legend.key.height = unit(.35, "cm"),legend.position = c(.9, .3))
...
```{r}
b <- ggplot() + geom_polygon(data = counties.df2, aes(x = long, y = lat, group = group, fill = Chronic_Use_Rate), color = "black", size = 0.05)+ coord_map() + scale_fill_distiller(palette = "Greens", labels = percent, breaks = pretty_breaks(n = 10)) + guides(fill = guide_legend(reverse = TRUE)) + theme_nothing(legend = TRUE) + labs(title = "Chronic Opiate Use Rate", fill = "") + theme(plot.title = element_text(size=20, family = "Times"),legend.key.height = unit(.35, "cm"),legend.position = c(.9, .3))
